...
<div class="max-w-3xl mx-auto">
    <div class="mb-8">
        ...
        <h1 class="text-3xl font-['Orbitron'] font-black mt-4">Create <span class="text-[#facc15]">Permission</span></h1>
    </div>

    <form action="{{ route('admin.permissions.store') }}" method="POST" class="bg-white/[0.03] border border-white/10 rounded-2xl p-6 lg:p-8 space-y-6">
        @csrf
        <div>
            <label class="block text-sm text-gray-400 mb-2">Permission Name *</label>
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-600 focus:outline-none focus:border-[#facc15]/50 focus:bg-white/10 transition-all" placeholder="e.g. edit articles">
